...
from .pattern_en import singularize
...
class CHAIR(object):
    ...
    def __init__(self, imids, coco_path):
        ...
        synonym_file = os.path.join(curr_dir, 'synonyms.txt')
...
def load_generated_captions(cap_file):
   #Read in captions
    ...
    caps = []
    imids = set()
    with jsonlines.open(cap_file, 'r') as f:
        caps = [cap for cap in f]
...
def evaluate_chair(cap_file):
    ...
    evaluator = CHAIR(imids=imids, coco_path=coco_path) 
