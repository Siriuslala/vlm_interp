...
class Controlled_Images(Dataset):
    def __init__(
        ...
        download=False, 
        subset='A',
        directions=[],  # choices: ["left", "right", "on", "under"]
    ):
        self.root_dir = root_dir
        if subset == 'A':
        ...
        if directions != []:
            self.dataset = [self.dataset[i] for i in range(len(self.dataset)) if any([f" {x} " in self.dataset[i]["caption_options"][0] for x in directions])]
        ...
        if self.subset == 'A':
            ...
            for i in range(len(self.dataset)):
                self.dataset[i]['image_path'] = os.path.join(image_dir, '/'.join(self.dataset[i]['image_path'].split('/')[2:]))
        else:
            for d in self.dataset:
                if 'left_of' in d['image_path']:
                    self.all_prepositions.append('left_of')
...
class Controlled_Images_collate_function:
    ...
    def __call__(self, samples):
        ...
        for i, sample in enumerate(samples):
            # shuffle the orders of options
            options = sample["caption_options"].copy()
            np.random.shuffle(options)
            correct_answer_idx = options.index(sample["caption_options"][0])
            ...
            samples[i].update({"answer": correct_answer})
        # print(samples)
...
class Controlled_Images_no_options_collate_function:
    def __init__(self, processor, vision_process_func=None, model_name=None):
        self.processor = processor
        self.vision_process_func = vision_process_func
        self.model_name = model_name
    ...
    def __call__(self, samples):
        ...
        for i, sample in enumerate(samples):
            correct_ans = sample["caption_options"][0]
            ans = ""
            directions = ["left", "right", "on", "under", "in front of", "behind"]
            for word in directions:
                if f" {word} " in correct_ans:
                    ans = word
                    break
            
            pattern = r"(A|an) (.*) (on|under|to the left of|to the right of|in front of|behind) (a|an) (.*)"
            match = re.search(pattern, correct_ans)
            if not match:
                print(correct_ans, ans)
                import pdb; pdb.set_trace()
            obj_0 = match.group(2)
            obj_1 = match.group(5)
            
            if ans in ["on", "under", "left", "right"]:  # for whatsup a
                instruction = f"Is the {obj_0} on/under/to the left of/to the right of the {obj_1}? Please choose the best answer from the four options: [on, under, left, right], and reply with only one word. \nYour answer is:"
            else:  # for whatsup b
                instruction = f"Is the {obj_0} in front of/behind/to the left of/to the right of the {obj_1}? Please choose the best answer from the four options: [In front of, Behind, Left, Right], and reply with only one word. \nYour answer is:"
            prompts.append(instruction)
            samples[i].update({"answer": ans})
        # print(samples)
        # import pdb; pdb.set_trace()
        ...
        if any(name in self.model_name for name in ["qwen"]):
            ...
            num_patches_list = [pixel_value.size(0) for pixel_value in pixel_values]
            pixel_values = torch.cat(pixel_values, dim=0)
            model_inputs = self.processor(prompts, return_tensors='pt', padding=True)
...
class COCO_QA(Dataset):
    ...
    def __getitem__(self, index):
        ...
        item = {"image_path": image_path, "caption_options": [test_case[1], test_case[2]]}
...
class COCO_QA_collate_function:
    def __init__(self, processor, vision_process_func=None, model_name=None):
        self.processor = processor
        self.vision_process_func = vision_process_func
        self.model_name = model_name
    ...
    def __call__(self, samples):
        ...
        for i, sample in enumerate(samples):
            ...
            # np.random.shuffle(options)
            correct_answer_idx = options.index(sample["caption_options"][0])
            correct_answer = ["A", "B"][correct_answer_idx]
            qn = "Please select the correct option and return only the letter of the option.\nYour choice is:" if "llava" in self.model_name else "Please answer with A or B.\nYour answer is:"
            instruction = f"""
            Please choose the best decription for the image from the following 2 options:\n
            A: {options[0]}\n
            B: {options[1]}\n
            {qn}
            """
